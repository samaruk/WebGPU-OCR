...
@group(0) @binding(1) var<storage, read_write> dilat : array<f32>;
@group(0) @binding(2) var<storage, read_write> erode : array<f32>;
...
fn main(@builtin(global_invocation_id) gid:vec3<u32>) {
  ...
  for(var dy=-R;dy<=R;dy++){for(var dx=-R;dx<=R;dx++){
    ...
    dMax=max(dMax,input[u32(ny)*W+u32(nx)]);
  }}
  dilat[y*W+x]=dMax;
  storageBarrier();
  var eMin=1.0;
  for(var dy=-R;dy<=R;dy++){for(var dx=-R;dx<=R;dx++){
    let nx=clamp(i32(x)+dx,0,i32(W)-1); let ny=clamp(i32(y)+dy,0,i32(H)-1);
    eMin=min(eMin,dilat[u32(ny)*W+u32(nx)]);
  }}
  erode[y*W+x]=eMin;
}