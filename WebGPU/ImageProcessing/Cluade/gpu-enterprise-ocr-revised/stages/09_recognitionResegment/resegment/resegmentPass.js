...
export function encodeResegmentPass(device, encoder, pipeline,
    confTex, cleanTex, outputTex, width, height) {
  ...
  const bg = device.createBindGroup({ layout: pipeline.getBindGroupLayout(0), entries: [
    { binding:0, resource: confTex.createView()   },
    { binding:1, resource: cleanTex.createView()  },
    { binding:2, resource: outputTex.createView() },
    { binding:3, resource: { buffer: uni }         },
  ]});
  const p = encoder.beginComputePass({ label:'resegmentPass' });
