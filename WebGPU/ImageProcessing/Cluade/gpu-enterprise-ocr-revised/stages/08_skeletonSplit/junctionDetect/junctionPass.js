...
export function encodeJunctionPass(device,encoder,pipeline,skelBuf,graphTex,outputTex,width,height){
  ...
  const bg=device.createBindGroup({layout:pipeline.getBindGroupLayout(0),entries:[
    ...
    {binding:1,resource:graphTex.createView()},
    {binding:2,resource:outputTex.createView()},
    {binding:3,resource:{buffer:uni}},
  ]});
  const p=encoder.beginComputePass({label:'junctionPass'});
