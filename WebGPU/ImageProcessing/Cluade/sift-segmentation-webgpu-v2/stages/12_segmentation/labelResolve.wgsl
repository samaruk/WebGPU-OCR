// labelResolve.wgsl — union-find step (4-connected)
...
fn root(idx:u32)->u32{
  var r=idx;
  for(var i=0u;i<64u;i++){let p=labels[r];if(p==0u||p==r+1u){break;}r=p-1u;}
  return r;
}
...
fn main(@builtin(global_invocation_id) gid:vec3<u32>) {
  ...
  let ra=root(idx);
  let offs=array<vec2<i32>,4>(vec2<i32>(1,0),vec2<i32>(0,1),vec2<i32>(-1,0),vec2<i32>(0,-1));
  for(var k=0u;k<4u;k++){
    let nx=i32(gid.x)+offs[k].x; let ny=i32(gid.y)+offs[k].y;
    if(nx<0||ny<0||u32(nx)>=u.width||u32(ny)>=u.height){continue;}
    let ni=u32(ny)*u.width+u32(nx);
    if(labels[ni]==0u){continue;}
    let rb=root(ni);
    if(ra!=rb){let lo=min(ra,rb);let hi=max(ra,rb);labels[hi]=lo+1u;atomicAdd(&changed,1u);}
  }
}
