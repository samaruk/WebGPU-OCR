// shaders/morph_horizontal.wgsl — 1D horizontal dilation
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  var maxV: f32 = 0.0;
  for (var dx: i32 = -kh; dx <= kh; dx++) {
    let nx = clamp(cx + dx, 0, i32(W) - 1);
    // foreground = 0 (text), we want to dilate text (0s)
    maxV = max(maxV, 1.0 - textureLoad(src, vec2<i32>(nx, cy), 0).r);
  }
  // Write back: dilated foreground = 0
  let v = 1.0 - maxV;
