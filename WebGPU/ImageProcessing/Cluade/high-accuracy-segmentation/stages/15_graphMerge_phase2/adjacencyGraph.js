/**
 * stages/15_graphMerge_phase2/adjacencyGraph.js
 * Phase 2 Graph Merge — strict normalized thresholds + MERGE_CANDIDATE gate.
 * Only merges pairs that passed Stage 14 splitDetector with sufficient confidence.
 * Threshold: gap < mergeStrictGapFactor × meanStrokeWidth
 */
import { adaptiveHeuristics } from "../../adaptive/adaptiveHeuristics.js";

export async function runGraphMergePhase2(gpuCtx, mergedLabels, splitDecisions, sws, params, registry) {
  const { labelTex, componentCount } = mergedLabels;
  if (componentCount < 2) return mergedLabels;
  ...
  for (const [key, decision] of splitDecisions) {
    if (!decision.merge) continue;
    if (decision.confidence < params.splitConfidenceGate) continue;
    ...
    if (normGap < params.mergeStrictGapFactor && angle < params.mergeAngleStrictDeg) {
      union(mergeMap, a, b);
    }
  }
  ...
  return { labelTex: newTex, componentCount: newCount };
}
...
function buildBoundingBoxes(labels, W, H, count) {
  ...
  for (let i = 0; i < labels.length; i++) {
    ...
    if (!boxes[id]) boxes[id] = {x1:x,y1:y,x2:x,y2:y};
      ...
      boxes[id].x1=Math.min(boxes[id].x1,x); boxes[id].y1=Math.min(boxes[id].y1,y);
