...
export class OpenKernel {
  constructor(device) { this.device = device; }
  async init() {
    const code = await loadShader(new URL('./open.wgsl', import.meta.url).href);
    this.pipeline = await getComputePipeline(this.device, 'morphOpen', code);
  }
  run(enc, inBuf, tmpBuf, outBuf, W, H, ks = 3) {
    const runP = (src, dst) => {
      ...
      const pass = enc.beginComputePass({ label: 'morphOpen' });
