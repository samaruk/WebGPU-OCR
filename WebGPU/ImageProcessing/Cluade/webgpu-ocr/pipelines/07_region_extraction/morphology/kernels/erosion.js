...
export class ErosionKernel {
  constructor(device) { this.device = device; }
  async init() {
    const code = await loadShader(new URL('./erosion.wgsl', import.meta.url).href);
    this.pipeline = await getComputePipeline(this.device, 'erosion', code);
  }
  run(enc, inBuf, outBuf, W, H, ks = 3) {
    ...
    const pass = enc.beginComputePass({ label: 'erosion' });
