...
export class FFLinear1Kernel {
  constructor(device) { this.device = device; }
  async init() {
    const code = await loadShader(new URL('./linear1.wgsl', import.meta.url).href);
    this.pipeline = await getComputePipeline(this.device, 'ffLinear1', code);
  }
  run(enc, xBuf, WBuf, bBuf, outBuf, S, inD, outD) {
    ...
    const pass = enc.beginComputePass({ label: 'ffLinear1' });
