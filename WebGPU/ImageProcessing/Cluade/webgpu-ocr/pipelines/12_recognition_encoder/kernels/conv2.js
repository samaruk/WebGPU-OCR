...
export class EncConv2Kernel {
  constructor(device) { this.device = device; this.name = 'conv2'; }

  async init() {
    const code = await loadShader(new URL('./conv2.wgsl', import.meta.url).href);
    this.pipeline = await getComputePipeline(this.device, 'conv2', code);
  }
  ...
  run(enc, inBuf, outBuf, wBuf, bBuf, inW, inH, outC, inC, kH = 3, kW = 3, stride = 1, pad = 1) {
    ...
    const pass = enc.beginComputePass({ label: 'conv2' });
