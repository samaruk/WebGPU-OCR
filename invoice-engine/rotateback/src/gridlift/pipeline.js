...
import {
  RESAMPLE, RESAMPLE_WARP, LUMINANCE, BOX_STATS_X, SAUVOLA_Y, MEDIAN3, SCHARR, ANGLE_HIST,
...
import { M as MAT, toWgslMat3 } from './rectify/index.js';
...
export class GridliftGpu {
  ...
  /** Uniform buffer holding the rectifying homography (mat3x3, std140). */
  warpUniform() {
    this._warpBuf ??= this.device.createBuffer({
      label: 'warpMatrix',
      size: 48,
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
    });
    return this._warpBuf;
  }
   ...
   * @param {{rectify?: {H: Float64Array, width: number, height: number}}} [opts]
   *   When `rectify` is given, stage 02 resamples through the homography
   *   instead of a plain box downsample, so the working image is already
   *   deskewed and de-keystoned. `H` maps SOURCE pixels to RECTIFIED
   *   full-resolution pixels; `width`/`height` are that rectified frame.
   *   Everything downstream then reports coordinates in the rectified frame,
   *   which is what `sourceBounds` means for a rectified run.
   * @returns {Promise<object>} raw geometry evidence for the CPU stages
   */
  async runGpuStages(source, opts = {}) {
    ...
    const imgW = source.width ?? source.codedWidth;
    const imgH = source.height ?? source.codedHeight;
    const rect = opts.rectify ?? null;

    // The "source frame" the geometry graph refers to: the rectified page when
    // rectifying, the original image otherwise.
    const srcW = rect ? rect.width : imgW;
    const srcH = rect ? rect.height : imgH;
    ...
    log.stop({ srcW: imgW, srcH: imgH, workW: w, workH: h, scale: +scale.toFixed(4), rectified: !!rect });
    ...
    log.start(2, rect ? 'decode+rectify' : 'decode+normalise');
    const rgba = pool.acquire(U32, STORAGE_USAGE, 'rgba');
    if (rect) {
      // Working-rectified -> source, which is what the shader needs to sample.
      const Hwork = MAT.mul(MAT.scale(scale), rect.H);
      const Hinv = MAT.inverse(Hwork);
      if (!Hinv) throw new Error('rectify: homography is singular');
      dev.queue.writeBuffer(this.warpUniform(), 0, toWgslMat3(Hinv));
      runner.run('resampleWarp', RESAMPLE_WARP, {
        ...base, i0: imgW, i1: imgH, i2: Math.max(2, cfg.resampleTaps),
        f0: cfg.normalise.gain, f1: cfg.normalise.bias,
      }, [texView, rgba, this.warpUniform()], grid2d);
    } else {
      runner.run('resample', RESAMPLE, {
        ...base, i0: imgW, i1: imgH, i2: cfg.resampleTaps,
        f0: cfg.normalise.gain, f1: cfg.normalise.bias,
      }, [texView, rgba], grid2d);
    }
    log.stop();
    ...
    return {
      ...
      imageWidth: imgW,
      imageHeight: imgH,
      rectified: rect ? { H: rect.H, width: rect.width, height: rect.height } : null,
      scale,
