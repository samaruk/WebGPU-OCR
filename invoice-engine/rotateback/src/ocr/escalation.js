...
export class EscalationController {
   ...
   */
  /**
   * @param {object} [options.cropper] a cropper that rectifies each ROI on its
   *   way to the recogniser. Supplied when the page was rectified, so OCR reads
   *   straight text cut from the untouched original.
   */
  async run(source, geometry, ctx, options = {}) {
    const t0 = Date.now();
    this.trace = [];
    const budget = this.budget;
    const cropper = options.cropper ?? null;
    ...
    if (baseRects.length && this.baseOcr && !(this.baseOcr instanceof NullOcrProvider)) {
      ocr = await this.baseOcr.recognize(source, baseRects, { cropper });
    ...
    if (
      ...
      if (table) {
        ...
        if (rect.w * rect.h + spent.pixels <= budget.maxPixels * 2) {
          const parsed = await this.sidecar.parseStructure(source, rect, { cropper });
