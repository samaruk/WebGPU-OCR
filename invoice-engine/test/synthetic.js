...
export class SyntheticPage {
  ...
  text(text, { x, y, size = 14, align = 'left' } = {}) {
    ...
    for (const ch of text) {
      ...
      // Descenders/ascenders vary the glyph box a little so line clustering is
      // not tested against artificially perfect input.
      const jitter = 'gjpqy'.includes(ch) ? 2 : 0;
      this.#glyph(cursor, y, cw, size - (ch === '.' || ch === ',' ? size - 3 : 0) + jitter);
      if (ch === '.' || ch === ',') {
        // punctuation sits on the baseline
        const g = this.components[this.components.length - 1];
        g.y = y + size - 3;
        g.y1 = g.y + g.h - 1;
        g.cy = g.y + g.h / 2;
      }
