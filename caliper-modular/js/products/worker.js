...
import { buildIndex, matchRows, searchProducts } from './matcher.js';
...
onmessage=e=>{
  const {id, rows, opts, search}=e.data||{};
  const t=performance.now();
  try{
    if(search) postMessage({id, results:searchProducts(index, search.text, search.limit||200), ms:Math.round(performance.now()-t)});   // the popup's ranked list
    else postMessage({id, results:matchRows(index, rows||[], opts||{}), ms:Math.round(performance.now()-t)});
  }
